...
int ggdecode(const string& encryption_value) {
  string encryption_key(GkEncryptionKey, kKeySize);
  string integrity_key(GkIntegrityKey, kKeySize);

  // This is an example of the encrypted data. It has a fixed length of 38
  // characters. The two padding characters are removed. It decodes to a string
  // of 28 bytes. The decrypted value should be 709959680.
  const string kB64EncodedValue(encryption_value);
  string encrypted_value = B64Decode(kB64EncodedValue);
  if (encrypted_value.size() != kEncryptedValueSize) {
    printf("Google Decode Error: unexpected ciphertext length: %lu\n",
           encrypted_value.size());
    return 1;
  }
  ...
  if (success) {
    ...
    printf("Google Decode Failed to decrypt value.\n");
    return value;
  }
  return value/10000;
}
